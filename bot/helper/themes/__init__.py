...
from bot.helper.themes import skmlx_minimal

AVL_THEMES = {}
for theme in listdir('bot/helper/themes'):
    if theme.startswith('skmlx_') and theme.endswith('.py'):
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].SKMLStyle(), var_name, None)
        if text is None:
        ...
        text = getattr(rantheme.SKMLStyle(), var_name, None)
        
    if text is None:
        text = getattr(skmlx_minimal.SKMLStyle(), var_name)
