...
async def gdcloneNode(message, link, listen_up):
    ...
    if is_gdrive_link(link):
        ...
        btn = ButtonMaker()
        btn.ibutton('Click Here to Read More ..', f'skmlx {message.from_user.id} help CLONE')
...
async def clone(client, message):
    ...
    if len(link) == 0:
        btn = ButtonMaker()
        btn.ibutton('Cʟɪᴄᴋ Hᴇʀᴇ Tᴏ Rᴇᴀᴅ Mᴏʀᴇ ...', f'skmlx {message.from_user.id} help CLONE')
